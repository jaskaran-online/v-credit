    ...
    return (<>
        ...
        >
            <View className="flex flex-row items-center w-1/4">
                <View className="mr-1">
                    {transaction?.transaction_type_id === 2 ? (<MaterialCommunityIcons
                            name="call-received"
                            size={14}
                            color="green"
            ...
            <View>
                {transaction?.transaction_type_id === 1 ? (<View className={"mr-2"}>
                        <Text variant={"bodyMedium"}
                              className="text-slate-800 mr-2">{parseFloat(transaction?.amount).toFixed(2)}</Text>
                        <Text variant={"labelSmall"} className="text-slate-400 mr-2">
                            (Udhaar)
                        </Text>
                    </View>) : (<Text variant={"bodyMedium"} className={"text-slate-400 text-center"}>
                        {" "}
                        -{" "}
                    </Text>)}
            </View>
            <View className={"flex flex-row items-right"}>
                <View className={"flex flex-row items-center mr-8"}>
                    {transaction?.transaction_type_id === 2 ? (<View>
                ...
                <Octicons name={expanded ? "chevron-up" : "chevron-down"} size={16} color="lightgray"
                          style={{marginRight: 5, paddingTop: 5}}/>
            </View>
        </TouchableOpacity>
        {expanded && <View className={"bg-blue-50 h-14 flex-row flex py-3 justify-end px-5 items-center gap-x-5"}>
            {isEditable && <>
                <TouchableOpacity
            ...
            {balanceType !== "clear" && <TouchableOpacity className="flex items-center gap-1" onPress={async () => {
                await Share.share({
                    message : message
                });
            }}>
...
export const renderHeader = () => (<View className={"flex-row justify-between px-4 py-2 space-x-2 items-center"}>
