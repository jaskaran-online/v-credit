...
const ShareScreen = ({route}) => {
    ...
    let transactionsGroupBy = _.groupBy((_.sortBy(transactionsData, ['dateOnly'])), 'dateOnly');
    ...
    for (const [date, transactions] of Object.entries(transactionsGroupBy)) {
        ...
        <td colspan="5" style="padding: 10px 8px; text-align: left; border-bottom: 1px solid #ddd;">
            <span style="font-size: 10px;max-width: 30px;font-weight: bold;">${formatDateForMessage(date)}</span>
        </td>
        </tr>`;
    ...
    <tr style="background: ${index % 2 === 0 ? 'white' : 'white'}">
        ...
        <span style="max-width: 30px">${(formatDateForMessage(transaction.date)).split(' ')[0]} ${(formatDateForMessage(transaction.date)).split(' ')[1]}</span></td>
        <td style="padding: 10px 8px; text-align: left; border-bottom: 1px solid #ddd;">${
                transaction.transaction_type_id === 1
                    ? 'Credit Given'
                    : 'Payment Received'
            }</td>
        <td style=" ${transaction.transaction_type_id === 1 ? 'background-color:rgba(255,167,154,0.28);' : null} padding: 10px 8px; text-align: left; border-bottom: 1px solid #ddd;">${
                transaction.transaction_type_id === 1
                    ? transactionAmount + ' ₹'
                    : ''
            }</td>
        <td style=" ${transaction.transaction_type_id === 1 ? null : 'background-color:#DDECD9FF;'}padding: 10px 8px; text-align: left; border-bottom: 1px solid #ddd;">${
                transaction.transaction_type_id === 1
                    ? ' '
                    : transactionAmount + ' ₹'
            }</td>
  ...
  <body style="position: relative;font-family: Arial, sans-serif; margin: 0;overflow: hidden; padding: 20px;">
    ...
    <div style="margin: 0 auto; padding: 20px;  border-radius: 5px;box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
      <div style="text-align: center; margin-bottom: 20px; margin-top: 20px;">
        ...
        <p style="font-size: 17px; margin: 5px;color: dimgrey;">(${formatDateForMessage(firstKey)} - ${formatDateForMessage(lastKey)})</p>
      </div>
