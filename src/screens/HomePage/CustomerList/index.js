...
function CardComponent({title, iconName, description, makeCall = () => null, whatsapp = () => null}) {
    return (
        <View
            className={
        ...
        >
            ...
            <View className={"flex flex-row gap-4"}>
                <TouchableOpacity className='bg-blue-100 p-2 rounded-full' onPress={makeCall}>
                    <MaterialCommunityIcons name='phone' size={18} color='dodgerblue'/>
                </TouchableOpacity>
                <TouchableOpacity className='bg-emerald-50 p-2 rounded-full' onPress={whatsapp}>
...
export default function Index({navigation}) {
    ...
    return (
        <View className='flex-1 justify-start bg-blue-50'>
            <FlashList data={customerList} renderItem={({item, index}) =>
                <CardComponent
                    key={index}
                    title={item.name}
                    iconName='person'
                    ...
                    whatsapp={() => sendWhatsAppMessage(`https://wa.me/91${item.digits}?text=Hello`)}
                />}/>
        </View>
    );
}
